...
const FlickeringGridBackground: React.FC<FlickeringGridBackgroundProps> = ({
  squareSize = 4,
  gridGap = 6,
  flickerChance = 0.3,
  color = 'rgb(0, 0, 0)',
  ...
  const memoizedColor = useMemo(() => {
    const toRGBA = (color: string) => {
      ...
      let r = ctx.getImageData(0, 0, 1, 1).data[0];
      let g = ctx.getImageData(0, 0, 1, 1).data[1];
      let b = ctx.getImageData(0, 0, 1, 1).data[2];
      return `rgba(${r}, ${g}, ${b},`;
    };
    return toRGBA(color);
  }, [color]);
